  ...
  try {
    const validationRules = [
      ...
      body("content_category")
        .notEmpty()
        .withMessage("Content Category is required"),
    ];
    ...
    if (!request.files || !("video" in request.files)) {
      return response
        .status(400)
        .json({ status: 400, message: "Please provide a video File." });
    }
    const videoFile = request.files.video;
    if (
      typeof videoFile.mimetype === "string" &&
      !videoFile.mimetype.startsWith("video/")
    ) {
      return response
        .status(400)
        .json({ status: 400, message: "Please provide a valid video file." });
    }
    let content_url;
    try {
      const contentUrl = await uploadVideo(
        videoFile[0].path,
        "lecture_podcast/videos/content_library_videos"
      );
    ...
    if (request.files.thumbnail) {
      try {
        thumbnail = await uploadImage(
          ...
          "lecture_podcast/thumbnails/content_library_thumbnails"
        );
...
async function createAudioContent(request, response) {
  ...
  try {
    ...
    if (!request.files || !("audio" in request.files)) {
      return response
        .status(400)
        .json({ status: 400, message: "Please provide an audio File." });
    }
    const audioFile = request.files.audio;
    let content_url;
    try {
      const contentUrl = await uploadAudio(
        audioFile[0].path,
        "lecture_podcast/audio/content_library_audios"
      );
...
async function createFileContent(request, response) {
  ...
  try {
    ...
    if (!request.files || !("document" in request.files)) {
      return response
        .status(400)
        .json({ status: 400, message: "Please provide a document file." });
    }
    ...
    if (!isDocument) {
      return response.status(400).json({
        status: 400,
        message: "Please provide a valid document file.",
      });
    }

    const content_url = await uploadFile(
      request.files["document"][0].path,
      "lecture_podcast/documents/content_library_documents"
    );
...
async function allContent(request, response) {
  ...
  try {
    ...
    if (!contents)
      return response
        .status(404)
        .json({ status: 404, message: "Lecturer has no content yet" });
    return response.status(200).json({
      status: 200,
      message: "Contents Fetch successfully",
      data: contents,
    });
  } catch (error) {
    console.log(error);
...
async function singleContent(request, response) {
  ...
  try {
    ...
    if (!content)
      return response
        .status(404)
        .json({ status: 404, message: "Content Not Found" })
  ...
  try {
    ...
    if (!check_owner) {
      return response.status(404).json({
        status: 404,
        message: "Content does not belong to this lecturer",
      });
    }
...
async function deleteContent(request, response) {
  ...
  try {
    ...
    // Perform the update operation
    const deletedContent = await ContentLibrary.findOneAndDelete({
      _id,
      lecturer_id,
    });
