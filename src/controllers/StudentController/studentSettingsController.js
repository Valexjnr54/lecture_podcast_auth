...
async function changePassword(request, response) {
    ...
    try {
        ...
        const update = await Student.findOneAndUpdate( {_id: student_id},{password: hashedPassword}, {new:true})
    ...
    try {
        ...
        if (request.file) {
            ...
            if (profile_image_path != null) {
                const uploadedImageUrl = await uploadImage(profile_image_path, 'lecture_podcast/images/student_passports');
                if (uploadedImageUrl) {
        ...
        const student_updated = await Student.findOneAndUpdate({ _id: student_id },{ profile_image }, {new:true} );
        return response.status(200).json({ message: 'Student profile image updated', data:student_updated });
    } catch (error) {
...
async function updateDetails(request, response) {
    ...
    try {
        ...
        return response.status(200).json({ message: 'Student profile updated', data:user });
